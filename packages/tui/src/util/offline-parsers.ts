...
export function resolveParserUrl(url: string, filetype: string, kind: string): string {
  ...
  if (kind === "wasm") {
    try {
      const wasmFiles = fs.readdirSync(path.join(parsersDir, filetype))
...
export interface ParserConfig {
  ...
  aliases?: string[]
  injectionMapping?: Record<string, Record<string, string>>
 ...
 * 此函数是幂等的，多次调用结果一致。
 */
export function resolveOfflineParsers(parsers: ParserConfig[]): ParserConfig[] {
  const parsersDir = getParsersDir()
