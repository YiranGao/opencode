#!/bin/bash
# ============================================================
# 灵犀代码 (LingxiCode) - 麒麟OS x64 baseline 一键编译脚本
# 运行环境: Win11 WSL2 Ubuntu
#
# 用途: 编译不使用 AVX2 指令的 baseline 版本，用于解决部分
#       虚拟机环境下 "Illegal instruction" 崩溃问题（虚拟机
#       CPUID 暴露 avx2 但 XCR0 未启用 AVX2 执行状态，Bun 的
#       AVX2 优化指令触发 #UD）。baseline 版仅要求 SSE2，任何
#       x64 CPU 均可运行，性能损失通常 <10%。
#
# 用法:
#   ./build-kylin-x64-baseline.sh              # 增量编译（默认）
#   ./build-kylin-x64-baseline.sh --full       # 全量编译（清理后重新开始）
#   ./build-kylin-x64-baseline.sh --full --clean-only  # 仅清理，不编译
#   ./build-kylin-x64-baseline.sh --skip-parsers       # 跳过解析器下载
#
...
BUILD_TARGET="opencode-linux-x64-baseline"
...
CUSTOM_TARGET="linux-x64-baseline"
SYNC_SOURCE=true
...
echo ""
echo "============================================================"
step "  灵犀代码 - 麒麟OS x64 编译 (baseline, 无 AVX2)"
echo "============================================================"
echo ""
echo "  模式:   $([ "$FULL_BUILD" = true ] && echo '全量编译' || echo '增量编译')"
echo "  目标:   $BUILD_TARGET"
echo "  目录:   $PROJECT_DIR"
echo "  说明:   baseline 版本兼容 AVX2 异常的虚拟机环境"
echo ""
...
if [ "$FULL_BUILD" = true ]; then
    info "  执行模型快照生成（离线时自动回退 offline-cache/models/api.json）..."
...
BINARY_PATH="dist/$BUILD_TARGET/bin/$BINARY_NAME"
    ...
    if echo "$BINARY_FILE" | grep -q "x86-64"; then
        ok "  编译成功: x86-64 baseline, $BINARY_SIZE"
    else
...
cat > "$BUILD_LOG" << BIEOF
{
  ...
  "platform": "linux-x64-glibc-baseline",
  "instructionSet": "baseline (no AVX2, SSE2 only)",
