#!/bin/bash
# ============================================================
# 灵犀代码 (LingxiCode) - macOS ARM64 (Apple Silicon) 一键编译脚本
# 运行环境: Win11 WSL2 Ubuntu（交叉编译 darwin 目标）
# 产物: packages/opencode/dist/opencode-darwin-arm64/bin/opencode
# 用法:
#   ./build-macos-arm64.sh              # 增量编译（默认）
#   ./build-macos-arm64.sh --full       # 全量编译（清理后重新开始）
#   ./build-macos-arm64.sh --full --clean-only  # 仅清理，不编译
#   ./build-macos-arm64.sh --skip-parsers       # 跳过解析器下载
#   ./build-macos-arm64.sh --target=darwin-arm64  # 仅编译指定目标
# ============================================================
...
BUILD_TARGET="opencode-darwin-arm64"
...
echo ""
echo "============================================================"
step "  灵犀代码 - macOS ARM64 (Apple Silicon) 编译"
echo "============================================================"
echo ""
...
if ls node_modules/.bun/@parcel+watcher-darwin-arm64* >/dev/null 2>&1; then
...
BINARY_PATH="dist/$BUILD_TARGET/bin/$BINARY_NAME"
    ...
    if echo "$BINARY_FILE" | grep -q "Mach-O" && echo "$BINARY_FILE" | grep -q "arm64"; then
        ok "  编译成功: Mach-O arm64, $BINARY_SIZE"
    else
...
cat > "$BUILD_LOG" << BIEOF
{
  ...
  "targetOS": "MacOS ARM64  (Apple Silicon)"
}
BIEOF
