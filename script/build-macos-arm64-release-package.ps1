<#
.SYNOPSIS
    打包 LINGXI CODE macOS ARM64 (Apple Silicon) 离线部署包
.DESCRIPTION
    将构建好的 darwin-arm64 二进制、离线解析器、配置文件、
    启动脚本打包为可直接在内网 macOS 上部署的 tar.gz 包。
.PARAMETER Target
    构建目标类型，需与 build-macos-arm64.sh 一致。默认 darwin-arm64。
.PARAMETER OutputDir
    输出目录。默认 <project-root>/dist-offline-macos-arm64
.EXAMPLE
    .\build-macos-arm64-release-package.ps1
    使用默认配置打包
.NOTES
    前置条件: packages/opencode/dist/opencode-darwin-arm64/bin/opencode
#>

param(
    [ValidateSet("darwin-arm64")]
    [string]$Target = "darwin-arm64",
    [string]$OutputDir
)
...
$ArtifactDir = Join-Path $ProjectRoot "artifacts\macos-arm64\release"
$ArtifactDirAttached = Join-Path $ProjectRoot "artifacts\macos-arm64\attached"

if (-not $OutputDir) {
    $OutputDir = Join-Path $ProjectRoot "dist-offline-macos-arm64"
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  LINGXI CODE macOS ARM64 Offline Packaging" -ForegroundColor Cyan
...
if (-not (Test-Path $binary)) {
    if (-not (Test-Path $binaryZip)) {
        Write-Error "macOS ARM64 binary not found: $binaryZip"
        exit 1
    }
    Expand-Archive -Path $binaryZip -DestinationPath $binaryBin -Force
    if (-not (Test-Path $binary)) {
        Write-Error "macOS ARM64 binary not found: $binary"
        exit 1
    }
}
...
$tarName = "lingxicode-offline-v${Version}-macos-arm64.tar.gz"
...
Write-Host ""
Write-Host "  Deploy to macOS (Apple Silicon):" -ForegroundColor Cyan
