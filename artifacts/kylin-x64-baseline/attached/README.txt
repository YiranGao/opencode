LINGXI CODE CLI - 企业内网离线版 v1.18.18 (baseline 版，无 AVX2 指令)
========================

本包为 baseline 版本：适用于部分虚拟机环境（CPUID 暴露 avx2 但未真正启用
AVX2 执行状态，标准版运行报 "Illegal instruction" 崩溃的机器）。
普通物理机建议使用标准版（kylin-x64）包，性能更优。
